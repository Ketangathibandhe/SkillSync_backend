...
`;
  ...
  // Convert raw markdown into structured JSON
  const structured = { missingSkills: [], learningPriorities: [] };
  try {
    ...
    lines.forEach(line => {
      if (/missing skills/i.test(line)) {
        currentSection = "missingSkills";
      } else if (/learning priorities/i.test(line)) {
        currentSection = "learningPriorities";
      } else if (/^\d+\.\s/.test(line) && currentSection) {
        // Remove number prefix and trim
        const item = line.replace(/^\d+\.\s*/, "").trim();
        if (item) structured[currentSection].push(item);
      }
    });
  } catch (err) {
    console.error("Skill gap parsing error:", err);
  }

  return { structured, rawText };
};

// Helper: Roadmap Generation (unchanged)
const getRoadmapFromModel = async (targetRole, skillGap) => {
  const prompt = `
Create a **comprehensive and detailed learning roadmap** for becoming a ${targetRole}.
Base it on the following skill gap:
...
const analyzeSkillGap = async (req, res) => {
  try {
    ...
    const { structured, rawText } = await getSkillGapFromModel(targetRole, currentSkills);

    res.json({
      success: true,
      skillGap: structured,
      rawText, // optional, in case frontend wants raw markdown
    });
  } catch (err) {
    console.error("Skill gap error:", err);
    res.status(500).json({ error: "Failed to analyze skill gap" });
...
const generateRoadmap = async (req, res) => {
  try {
    ...
    const roadmap = new Roadmap({
      ...
      targetRole,
      currentSkills,
      skillGap: skillGap.structured, // store structured version
      steps,
      rawText,
      progress: 0, //  initial progress
    });
